...
class PowerRAGTool(BaseTool):
    ...
    def __init__(self, index_path: str):
        ...
        try:
            ...
            QUERY_PROMPT = ChatPromptTemplate.from_messages([
                ("system", 
                 "당신은 전력 관련 검색 쿼리를 생성하는 AI입니다. 주어진 질문을 기반으로 검색 쿼리를 만드세요.\n\n"
                 "매우 중요한 규칙:\n"
    ...
    def _extract_core_query(self, query: str, region: str) -> str:
        ...
        # 지역명과 회사명 패턴들
        region_patterns = [
            region, f"{region}전력", f"{region}한전", f"{region}지사", f"{region}지역",
            "전력", "한전", "지사", "지역", "회사", "공사", "발전소"
        ]
            ...
            try:
                ...
                for msg in chat_history[-3:]:  # 최근 3개 메시지만 고려
                    if isinstance(msg, dict):
                        ...
                        # 주요 도메인 키워드 추출 (전력 관련)
                        domain_keywords = ['계약', '연체료', '요금', '신청', '승인', '통지', '책임', '준수', '전력', '발전', '송전']
