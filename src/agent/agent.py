...
class Agent:
    ...
    def __init__(self, vectorstore_path: str):
        ...
        # 각 Agent 초기화
        self.legal_agent = LegalAgent(vectorstore_path)
    ...
    def run(self, question: str, chat_history: Optional[List] = None) -> str:
        ...
        # 질문을 처리할 수 있는 Agent 찾기
        for agent in self.agents:
            if agent.can_handle(question):
                return agent.run(question, chat_history)
        
        # 처리할 수 있는 Agent가 없는 경우 일반 Agent 사용
        return self.general_agent.run(question, chat_history)

    async def get_response(self, messages: List[Dict[str, str]]) -> str:
        """
        주어진 메시지 목록을 기반으로 AI 응답을 생성합니다.
        법령 관련 질문인 경우 LegalAgent를 사용하고, 그 외의 경우 일반 대화를 처리합니다.
        ...
        try:
            ...
            if last_user_message:
                ...
                for agent in self.agents:
                    if agent.can_handle(last_user_message):
                        response = agent.run(last_user_message, chat_history)
                        logger.info(f"Agent Response: {response}")
                        return response
            
            # 법령 관련 질문이 아니거나 Agent가 없는 경우 기본 응답 생성
