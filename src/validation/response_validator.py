...
이유: [간단한 설명]"""
    ...
    def validate_response_quality(self, query: str, search_results: str, response: str) -> Tuple[bool, float]:
        ...
        """
        try:
            # 조문 번호가 언급된 경우 정확성 검증 강화
            article_validation = ""
            if any(keyword in query for keyword in ["조", "항", "제"]):
                article_validation = """
6. 특정 조문이나 조항이 언급된 경우, 응답이 해당 조문의 정확한 내용을 반영하고 있는가?
7. 참고 문서에 없는 일반적인 법률 지식이나 추측이 포함되어 있지 않은가?
8. 조문의 각 항(①, ②, ③, ④)이 정확히 구분되어 설명되고 있는가?"""
...
4. 응답이 명확하고 이해하기 쉬운가?
5. 응답이 참고 문서의 범위를 벗어나지 않는가?{article_validation}

**특별 검증 사항:**
- 참고 문서에 없는 내용이 응답에 포함되어 있으면 감점
- 조문 번호가 질문에 있는 경우, 해당 조문의 정확한 내용만 다뤄야 함
- 일반적인 법률 지식이나 추측이 포함되어 있으면 감점
